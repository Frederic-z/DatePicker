(function() {
	...
	function calendar(id, period, inputNum) {
		...
		this.dateInfo = {
			...
			getTheMonth: ["JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"],
			getWeekString: ["Su", "Mo", "To", "We", "Th", "Fr", "Sa"]
	...
	calendar.prototype = {
		...
		init: function() {
			...
			if(this.inputNum === 2) {
				inputs = '<input type="text" class="twoInput">' + '<b>to</b' + '<input type="text" class="towInput">';
			} else {
		...
		refresh: function() {
			...
			for (var i = 0,len = tds.length; i < len; i++) {
				...
				if (tds[i].innerHTML !== "" && (new Date(selectYear + ", " + (selectMon + 1) + ", " + Number(tds[i].innerHTML))) >= now) {
					//  绑定点击返回事件，经过时的色彩变化
		...
		//	点下日期后的回调函数
		chosennd: function() {
			var cal = document.getElementsByClassName("calender" + this.id)[0];
			...
			var content1 = this.choice1.getFullYear() + "/" + this.choice1.getMonth() + "/" +
			this.choice1.getDate();
			var content2 = this.choice2.getFullYear() + "/" + this.choice2.getMonth() + "/" +
			this.choice2.getDate();
			//找到最后被允许的那个
		...
		showCal: function() {
			var that = this;
			var cal = document.getElementsByClassName("calendar" + this.id)[0];
			var inputCal = cal.getElementsByClassName("form-cal")[0].getElementsByTagName("button")[0];
